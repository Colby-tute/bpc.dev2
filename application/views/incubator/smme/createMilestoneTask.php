...
<div class="container">
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
			<div class="col-sm-12">
				<input type="hidden" id="ediTaskId" name="title" placeholder="Id" class="form-control task_id"/>
				<label>Task Name</label>
				<input type="text" id="ediTaskName" name="title" placeholder="Task Name" class="form-control task_name"/>
			</div><br />
			<div class="col-sm-12">
				<label>Task Description</label>
				<input type="text" id="ediTaskDesc" name="taskDesc" placeholder="Task Description" class="form-control task_description"/>
			</div><br />
			<div class="col-sm-12">
				<label>Task Target Date</label>
				<input type="text" id="ediTaskEndDate" name="end_date" placeholder="Task End date"
					   class="form-control datepicker task_target_date"/>
			</div>
	      </div>
	      <div class="modal-footer">
	        <button type="button" id="close_task_edit" class="btn btn-secondary" data-dismiss="modal">Close</button>
	...
	<div class="breadcrumb-header justify-content-between">
		<div>
			<h4 class="content-title mb-2">SMME
			 / <?= $smme->tbl_users_firstname ?> <?= $smme->tbl_users_lastname ?> / <?= $smme->tbl_users_user_uniqueid ?>
		    </h4>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb">
					<li class="breadcrumb-item"><a href="<?= site_url('bdsp/home') ?>">Dashboard</a></li>
					<li class="breadcrumb-item">SMME</li>
					<li class="breadcrumb-item active" aria-current="page">Create Milestone Task</li>
				</ol>
			</nav>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function createMilestoneTask() {
		...
		if(name == "" || desc == "" || endDate == "" || smmeId == "") {
			...
			$.ajax({
				method: "post",
				url: '<?= base_url() ?>bdsp/Smme/processMilestoneTasks/'+smmeId,
				data: {
	...
	function editMilestoneTask() {
		let taskId = $("#ediTaskId").val();
		let taskName = $("#ediTaskName").val();
		let taskDesc = $("#ediTaskDesc").val();
