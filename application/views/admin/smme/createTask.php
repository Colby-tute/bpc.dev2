	...
	<div class="modal-dialog modal_small" role="document">
		<div class="modal-content modal-content-demo">
			<div class="modal-body feedback_user_id">
				...
				<button class="btn btn-primary" style="margin-top: 15px" onclick="updateElement()" type="button">Edit</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	$(function () {
		...
		var KanbanTest = new jKanban({
			...
			dropEl: function (el, target, source, sibling) {
				...
				$.ajax({
					method: "POST",
					url: path + "admin/smme/Smme/orderElements",
					data: {
			...
			buttonClick: function (el, boardId) {
				...
				formItem.addEventListener("submit", function (e) {
					...
					$.ajax({
						method: "POST",
						url: path + "admin/smme/Smme/saveElement",
						data: {
							brdID: boardId,
							title: text,
							index: 1,
		...
		addBoardDefault.addEventListener("click", function (event) {
			...
			$.ajax({
				method: "POST",
				url: path + "admin/smme/Smme/saveBoard",
				data: {
					title: $("#board_title").val(),
					smmeId: "<?= $smme_id ?>",
				}
	...
	function removeElement(elem) {
		...
		$.ajax({
			method: "POST",
			url: path + "admin/smme/Smme/deleteElement",
			data: {
				id: element.text(),
	...
	function updateElement() {
		...
		$.ajax({
			method: "POST",
			url: path + "admin/smme/Smme/updateElement",
			data: {
				id: element.text(),
	...
	function deleteBoard(elem) {
		...
		$.ajax({
			method: "POST",
			url: path + "admin/smme/Smme/deleteBoard",
			data: {
				boardId: id
			}
		})
