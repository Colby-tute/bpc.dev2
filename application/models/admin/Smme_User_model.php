 ...
 */
class Smme_User_model extends CI_Model
{
    ...
    public function msme_directory_get_all($key =null) {
        if($key == null){
            $this->db->join('tbl_roles', 'tbl_roles.tbl_roles_id = tbl_users.tbl_users_role_id','LEFT');
            $this->db->where('tbl_users_role_id', 2);
            $this->db->group_by('tbl_users_id');
            ...
            $sql = "SELECT * FROM `tbl_users` LEFT JOIN `tbl_roles` ON `tbl_roles`.`tbl_roles_id` = `tbl_users`.`tbl_users_role_id` WHERE (tbl_users_firstname like '%".$key."%' OR tbl_users_lastname like '%".$key."%' OR `tbl_users_user_uniqueid` LIKE '%".$key."%' OR `tbl_users_mobile` LIKE '%".$key."%' OR `tbl_users_email` LIKE '%".$key."%') AND `tbl_users_role_id` = 2 GROUP BY `tbl_users_id`";
    ...
    public function did_delete_company_row($i)
    {
		...
		// Asssign MSME to Main MSME
		// application
		$dataSmme1['tbl_application_smme_id'] = null;
		$this->db->where('tbl_application_smme_id', $i);
        $query=$this->db->update('tbl_application', $dataSmme1); //check and delete

		$dataSmme11['smme_id'] = null;
        $this->db->where('smme_id', $i);
        $query=$this->db->update('tbl_application_assignment', $dataSmme11);//check and delete

		$dataSmme2['user_id'] = null;
		$this->db->where('user_id', $i);
        $query=$this->db->update('tbl_smme_event', $dataSmme2);//check and delete
		
		$dataSmme3['tbl_feedback_smme_unique_id'] = '0000';
		$this->db->where('tbl_feedback_smme_unique_id', $result->tbl_users_user_uniqueid);
        $query=$this->db->update('tbl_feedback', $dataSmme3);
		
		$dataSmme4['smme_id'] = '0000';
		$this->db->where('smme_id', $i);
        $query=$this->db->update('tbl_evaluate_smme_inc', $dataSmme4);
		
		$dataSmme5['facility_booking_user_id'] = '0000';
		$this->db->where('facility_booking_user_id', $i);
        $query=$this->db->update('tbl_facility_bookings', $dataSmme5); //delete
		...
		if($num_rows == 0)
		{
            ...
            //delete application
            $this->db->where(['tbl_application_smme_id'=>null,'tbl_application_incubator_id'=>null,'tbl_application_bdsp_id'=>null])->delete('tbl_application');
            //delete event
            $this->db->where(['user_id'=>null,'bdsp_id'=>null,' incubator_id'=>null])->delete('tbl_smme_event');
            //delete application relation
            $this->db->where(['smme_id'=>null,'incubator_id'=>null,'bdsp_id'=>null])->delete('tbl_application_assignment');
            //delete facility bookings
            $this->db->where('facility_booking_user_id','0000')->delete('tbl_facility_bookings');
		}
		else
		{
			// Asssign MSME to Main MSME
	...
	public function select_user_personal_data2($id) {
    ...
    public function select_user_data2($id) {
    ...
    public function select_user_business_data2($id) {
		$this->db->join('tbl_industry','tbl_industry.tbl_industry_id = tbl_business_details.tbl_business_details_industry','LEFT');
        $this->db->join('tbl_sub_industry','tbl_sub_industry.tbl_sub_industry_id = tbl_business_details.tbl_business_details_sub_industry','LEFT');
	...
	public function select_smme_teams_data2($id) {
