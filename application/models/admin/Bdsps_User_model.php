 ...
 */
class Bdsps_User_model extends CI_Model
{
   ...
   public function update_master($user,$personal,$business,$smme_teams,$upd_id,$persona_id,$business_id) {
    ...
    public function did_delete_company_row($i)
    {
		...
		// application
		$dataSmme1['tbl_application_bdsp_id'] = null;
		$this->db->where('tbl_application_bdsp_id', $i);
        $query=$this->db->update('tbl_application', $dataSmme1);

		$dataSmme11['bdsp_id'] = null;
        $this->db->where('bdsp_id', $i);
        $query=$this->db->update('tbl_application_assignment', $dataSmme11);//check and delete
		//
		$dataSmme2['bdsp_id'] = null;
		$this->db->where('bdsp_id', $i);
