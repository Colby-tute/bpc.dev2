...
class Evaluations_Model extends CI_Model
{
    ...
    public function get_smme_applications($smmeid)
    { 
        $this->db->distinct();
        $this->db->select('app_id,smme_id,u.tbl_users_firstname, u.tbl_users_lastname');
        $this->db->where('`app_id` IN (SELECT `app_id` FROM `tbl_application_assignment` WHERE `smme_id` = '.$smmeid.')', NULL, FALSE);
    ...
    public function get_smme_application($appid)
    { 
        $this->db->select('*');
