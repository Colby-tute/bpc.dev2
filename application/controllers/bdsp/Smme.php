...

{
	...
	public function editMilestone($id)
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			redirect(site_url("/bdsp/Smme/createMilestone/".$data['milestone'][0]->smme_id."/true"), 'refresh');
		}
		...
		$this->load->view('bdsp/smme/edit-milestone', $data);
	}
	...
	public function createMilestone($i, $flag = false)
	{
		...
		$this->load->view('bdsp/smme/milestone', $data);
	}
	...
	public function viewdetails($id) {
		...
		$this->load->view('bdsp/smme/viewdetails', $data);
	...
	public function createTask($i)
	{
		...
		$this->load->view('bdsp/smme/createTask', $data);
	}
	...
	public function createMilestoneTask($i)
	{
		...
		$this->load->view('bdsp/smme/createMilestoneTask', $data);
    ...
    public function handleMilestoneQuestions($id)
    {
        ...
        $this->load->view('bdsp/smme/handleMilestoneQuestions', $data);     
    }
    ...
    public function processQuestions($id) {
        ...
        if($questionAction == "getQuestions") {
			...
			$this->db->select("smme_id as id");
			$this->db->where("bdsp_id = " . $this->session->userdata("id_user") . " AND bdsp_id IS NOT NULL");
			$query = $this->db->get('tbl_application_assignment');
			$smmeIds2 = $query->result();
	...
	{
		if ($this->input->server("REQUEST_METHOD") == "POST") {
			$this->db->insert("tbl_evaluate", $_POST);
			redirect("bdsp/Operations/index");
		}
		...
		$this->db->select("tbl_application_incubator_id as id");
		$this->db->where("tbl_application_smme_id", $id);
		$incubatorID = $this->db->get("tbl_application")->result();

		$this->db->select("tbl_users_id as id, tbl_users_firstname as name, tbl_users_lastname as last_name");
		$this->db->where("tbl_users_id", $incubatorID[0]->id);
		$data['incubator'] = $this->db->get("tbl_users")->result();

		$this->db->select("tbl_users_id as id, tbl_users_firstname as name, tbl_users_lastname as last_name");
		$this->db->where("tbl_users_id", $id);
		$data['smme'] = $this->db->get("tbl_users")->result();

		$this->load->view('bdsp/smme/evaluate', $data);
	}
	...
	public function saveElement()
	{
		...
		$this->db->where('tbl_board_smme_unique_id', $_POST['smme_id']);
		$query = $this->db->get("tbl_board");
		$board = $query->result();
	...
	public function saveBoard()
	{
		$this->db->insert("tbl_board", [
			"tbl_board_user_unique_id" => $this->session->userdata("id_user"),
