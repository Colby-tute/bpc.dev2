...

{
	...
	public function editMilestone($id)
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			return $this->createMilestone($data['milestone'][0]->smme_id, true);
		}
		...
		$this->load->view('admin/smme/edit-milestone', $data);
	}
	...
	public function createMilestone($i, $flag = false)
	{
		...
		$this->load->view('admin/smme/milestone', $data);
	}
    ...
    public function handleMilestoneQuestions($id)
    {
        ...
        $this->load->view('admin/smme/handleMilestoneQuestions', $data);     
    }
	...
	public function get_sub_industries($id) {
		$industry_id = $id;
		// print_r($industry_id);
		$result = $this->Smme_User_model->sub_industries($industry_id);
	...
	public function createTask($i)
	{
		...
		$this->load->view('admin/smme/createTask', $data);
	}
	...
	public function get_all_industries() {
		$industrys = $this->Smme_User_model->select_industry();
		echo json_encode($industrys);
	}
	...
	public function saveElement()
	{
		...
		$this->db->select('tbl_board_id as id');
		$this->db->where('tbl_board_title', $brdid);
	...
	public function delete($i)
	{
		...
		if ($result == 1) {
			...
			$this->session->set_flashdata('success', 'The MSME has been deleted successfully!');

			redirect('admin/smme');

		} elseif ($result == 0) {
		
			$this->db->where('tbl_application_smme_id', $i);
			$query=$this->db->get('tbl_application');
			$num_rows=$query->num_rows();

			$this->session->set_flashdata('danger', 'This MSME has an active '.$num_rows.' application. Please delete application before you delete MSME!');
	...
	public function changeStatus()
	{
		...
		$this->db->update("tbl_users", [
			"tbl_users_status" => (string)$this->input->post("status")
		]);
	}
	...
	public function changeRights()
	{
		$this->db->where("tbl_users_id", $this->input->post("id"));
		$this->db->update("tbl_users", [
			"tbl_users_role_id" => $this->input->post("role")
		]);
	}

	public function createMilestoneTask($id)
	{
		$smme_id=$id;
		$this->db->select('*');
		...
		$this->db->select("*");
		$this->db->where('tbl_users_id', $smme_id);
		$query = $this->db->get("tbl_users");
		$data['smme'] = $query->result()[0];		

		$this->load->view('admin/smme/createMilestoneTask',$data);
	}
    ...
    public function viewFunds($smmeId) {
        ...
        $this->load->view('admin/smme/viewFunds',$data);
    }
	 ...
	 /* public function viewdetails($id) {
        ...
        $this->load->view('admin/smme/viewdetails',$data);*/
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$newInc =  $this->input->post('incubators') ? $this->input->post('incubators') : [];
			$this->update_incubator_coaches($app_id, $id, $newCoaches, $newInc);
			...
			if(sizeof($newInc) == 1) {
		        ...
		        $incubator_id = $result[0]->incubator_id;

				$this->db->select("*");
		        $this->db->where_in("user_id", [$smme_id, $incubator_id]);
		        ...
		        } else if ($inc_users_count == 1 && $result[0]->user_id == $incubator_id) {
	        		$incdata = array(
	                    'user_id' => $smme_id,
			...
			// $this->db->select("incubator_id as id");
			// $this->db->where("app_id", $app_id);
			// $this->db->where("incubator_id IS NOT NULL");
			// $query = $this->db->get("tbl_application_assignment");
			...
			// 		$this->db->where("app_id", $app_id);
			// 		$this->db->where("incubator_id", $ch_id);
			// 		$this->db->delete("tbl_application_assignment");
			// 	}
			// }
		...
		$data['userdt'] = $userdt;
		//$this->db->select("tbl_application_id as id");
		$this->db->where("tbl_application_id", $id);
		$result = $this->db->get("tbl_application")->result();
		if(!empty($result)){
			foreach($result as $row){
				$appid=$row->tbl_application_id;
			}
			$data["app_id"]=$appid;
		}else{
		$data["app_id"]=0;
    ...
    public function submitEditFunds() {
        ...
        redirect("admin/smme/Smme/viewFunds/" . $smmeId, 'location');
    }	
